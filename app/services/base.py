...
class BaseService(ABC):
    ...
    async def get(self, search_by: str = "username", search_value: str = ""):
        """
        Get a document from MongoDB service.
        """
        # key = list(search_value.keys())[0]
        # value = search_value[key]
        # return await db_mongo.get(
        #     self.collection_name, key, value, self.model_cls
        # )
        return await db_mongo.get(
            self.collection_name, search_by, search_value, self.model_cls
        )
    ...
    async def update(
        ...
        search_value: str = "",
        new_data: BaseModel = None,
    ):
        """
        Update a document in MongoDB by object ID.
        """
        return await db_mongo.update(
            self.collection_name, search_by, search_value, new_data
        )
    ...
    async def delete(self, username: str) -> Dict:
        ...
        """
        return await db_mongo.delete(self.collection_name, username)
